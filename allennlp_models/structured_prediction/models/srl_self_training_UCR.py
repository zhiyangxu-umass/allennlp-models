...
@Model.register("srl_self_training_UCR")
class SrlSelfTrainingFilter(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i, label in all_labels.items():
            if label[:3] == "B-A" and label[-1].isnumeric():
                core_roles.append(label[2:])
        self.core_roles = set(core_roles)
    ...
    def explore(self, wordpiece_tags, pred_idx, logis):
        # probs: (sequence len, labels num)
        ...
        if wordpiece_spans is None or len(wordpiece_spans)<2:
            ...
            for span in wordpiece_spans:
                if not span[2] in self.core_roles:
                    continue
                if span[2] in label2span:
                    repeat_labels.add(span[2])
                    label_idx = self.vocab.get_token_index('B-'+span[2],namespace="labels")
                    # print('B-'+span[2], label_idx)
                    label2span[span[2]].append(tuple([span[0],span[1],label_idx]))
                else:
                    label_idx = self.vocab.get_token_index('B-'+span[2],namespace="labels")
                    label2span[span[2]]=[tuple([span[0],span[1],label_idx])]
            for _label in list(repeat_labels):
                list_span_label_idx = label2span[_label]
                max_prob = float('-inf')
                max_span = None
                for span_label_idx in list_span_label_idx:
                    # print(span_label_idx)
                    # print(logis.shape)
                    curr_prob = logis[span_label_idx[0],span_label_idx[2]]
                    if max_prob < curr_prob:
                        max_prob = curr_prob
                        if max_span is not None:
                            wordpiece_tags[max_span[0]:max_span[0]+max_span[1]] = max_span[1]*['O']
                            pred_idx[max_span[0]:max_span[0]+max_span[1]] = max_span[1]*[self.vocab.get_token_index('O',namespace="labels")]
                            max_span = span_label_idx
                        else:
                            max_span = span_label_idx
                    if max_prob > curr_prob:
                        assert max_span is not None
                        wordpiece_tags[span_label_idx[0]:span_label_idx[0]+span_label_idx[1]] = span_label_idx[1]*['O']
                        pred_idx[span_label_idx[0]:span_label_idx[0]+span_label_idx[1]] = span_label_idx[1]*[self.vocab.get_token_index('O',namespace="labels")]
            # print('after filter')
            # print(wordpiece_tags)
            # print(pred_idx)
    ...
    def get_pred_tags(self, output_dict: Dict[str, torch.Tensor]):
        ...
        for predictions, length, offsets in zip(
            ...
            tags, max_likelihood_sequence = self.explore(tags, max_likelihood_sequence, predictions[:length])
