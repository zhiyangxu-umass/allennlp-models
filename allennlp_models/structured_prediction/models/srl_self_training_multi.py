...
@Model.register("srl_self_training_multi")
class SrlSelfTrainingBaseline(Model):
    ...
    def score_function(self, pred_spans, parse_spans):
        parse_score, use_parse = self.parse_vio(pred_spans, parse_spans) #[-num_pred_spans, 1]
        if use_parse:
            UCR_score, use_UCR = self.UCR_vio(pred_spans) #[-(num_pred_spans-1),0]
            if use_UCR:
                # print('use UCR')
                # print(pred_spans)
                return UCR_score, True
            # print('use parse')
            # print(pred_spans)
            return parse_score, True
        else:
            return 0.0, False

    def parse_vio(self, pred_spans, parse_spans):
        total_num_span = len(pred_spans)
        pred_longer_1 = set([(span[0],span[1]) for span in pred_spans if span[1]>1])
        if len(pred_longer_1) < 1:
            return 0.0, False
        else:
            num_violate_span = len(pred_longer_1 - pred_longer_1.intersection(parse_spans))
            if num_violate_span > 0:
                ...
                if total_num_span <=3:
                    ...
                    score = - math.exp(-1/4*(total_num_span-1))
            # print('current score',score)
            return score, True

    def UCR_vio(self, pred_spans):
        num_UCR_vio = 0
        label2span = {}
        for span_label in pred_spans:
            if not span_label[2] in label2span:
                label2span[span_label[2]]=[(span_label[0],span_label[1])]
            else:
                label2span[span_label[2]].append((span_label[0],span_label[1]))
        for label, span_list in label2span.items():
            if label in self.core_roles and len(span_list)>1:
                num_UCR_vio += len(span_list)-1
        if num_UCR_vio==0:
            return 0, False
        else:
            return 1, True
